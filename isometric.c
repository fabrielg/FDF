...
static void	apply_iso_to_point(t_fdf *fdf, t_point3 *src, t_point2 *dst)
{
	...
	x = src->v.axis[X];
	y = src->v.axis[Y];
	z = (float)(src->v.axis[Z]) * fdf->projection.z_divisor;
	dst->v.axis[X] = (x - y) * cos_a * fdf->projection.scale
		+ fdf->projection.offsets[X];
	dst->v.axis[Y] = (x + y) * sin_a * fdf->projection.scale
		- z * fdf->projection.scale / 2 + fdf->projection.offsets[Y];
}
